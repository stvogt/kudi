...
def general_print(filename, **kwargs):
  ...
  for j in range(0,iter_range):
    f.write('%2d' % (j+1)) 
    f.write('%15.2f' % (round(float(kwargs[key][j]),2)))
    for key in kwargs:
      if key != "Reaction Coordinate":
        f.write('%20.6f' % (round(float(kwargs[key][j]),6)))
    f.write('\n')
...
def general_plot(plotname, ylabel, limit_list, interval, bullets,  works, Zeroline, Show, **kwargs):
  count = 0
...
def general_plot_props(plotname, ylabel, proplist, limit_list ,interval, bullets, works, Zeroline, Show, **kwargs):
#def general_plot_prop_with_limits(plotname, ylabel, proplist, limit_list ,interval , Zeroline, Show, **kwargs):
  count = 0
  plt.rc('text', usetex=True)
  plt.rc('font', family='serif')
  ...
  for key in kwargs:
    if key == "Reaction Coordinate":
      ...
      plt.xlabel(r'$\mathbf{\xi}$ (a$_{0}$amu$^{\frac{1}{2}}$)', fontsize = 18)
      plt.xticks(fontsize = 18)
      #plt.xlabel(r'\huge{ $\mathbf{\xi}$ (amu$^{\frac{1}{2}}$)}')
  for key in proplist:
    if key != "Reaction Coordinate":
  ...
  print 'Saving the plot in ./figures as '+plotname
  print '----------------------------------------------------------'
  #plt.ylabel(r' \huge{'+ylabel+'}')
  plt.savefig(plotname)
  if Show:
    plt.show()
  plt.clf()

def plot_multi(plotname, ylabel, prop_list, limit_list, interval, bullets, works, Zeroline, Show):
  keys = []
  count = 0
  plt.rc('text', usetex=True)
  plt.rc('font', family='serif')
...
def general_plot_prop_with_cuts(plotname, ylabel, proplist, limit_list1, limit_list2, interval1, interval2, bullets, works, Show, **kwargs):
  ...
  for key in proplist:
    if key != "Reaction Coordinate":
      axarr[count].plot(x, kwargs[key], bullets[count], label=key, markeredgecolor="none")
  ...
  axarr[0].tick_params(labeltop='off',labelsize=18) # don't put tick labels at the top
...
def general_plot_scales(plotname, ylabel1, ylabel2, dictos, proplist, limit_list1, limit_list2, interval1, interval2, works, zeroline, Show ):
  # ONLY WORKS FOR TWO DIFFERENT PORPERTIES!!!
