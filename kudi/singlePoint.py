...
def bonddistance(lines, dis_list):
    ...
    def distance_calc(i, j):
        ...
        bnd_distance = np.sqrt(
            (float(xCoord[i]) - float(xCoord[j])) ** 2
            + (float(yCoord[i]) - float(yCoord[j])) ** 2
            + (float(zCoord[i]) - float(zCoord[j])) ** 2
        )
    ...
    if (
        dis_list
    ):  # If list is not empty, user specified a list of bond lengthes in object definition'
        for bl in dis_list:
...
def angle(lines, ang_list):
    ...
    def angle_calc(j, i, k):
        ...
        v_ij = np.array(
            [
                float(xCoord[i]) - float(xCoord[j]),
                float(yCoord[i]) - float(yCoord[j]),
                float(zCoord[i]) - float(zCoord[j]),
            ]
        )
        v_ik = np.array(
            [
                float(xCoord[i]) - float(xCoord[k]),
                float(yCoord[i]) - float(yCoord[k]),
                float(zCoord[i]) - float(zCoord[k]),
            ]
        )
...
def oop_angle(lines, oop_list):
    ...
    def oop_calc(i, j, k, l):
        ...
        v_kl = np.array(
            [
                float(xCoord[k]) - float(xCoord[l]),
                float(yCoord[k]) - float(yCoord[l]),
                float(zCoord[k]) - float(zCoord[l]),
            ]
        )
        v_ki = np.array(
            [
                float(xCoord[k]) - float(xCoord[i]),
                float(yCoord[k]) - float(yCoord[i]),
                float(zCoord[k]) - float(zCoord[i]),
            ]
        )
        v_kj = np.array(
            [
                float(xCoord[k]) - float(xCoord[j]),
                float(yCoord[k]) - float(yCoord[j]),
                float(zCoord[k]) - float(zCoord[j]),
            ]
        )
...
def dihedral(lines, dihed_list):
    ...
    def dihedral_calc(i, j, k, l):
        ...
        v_jk = np.array(
            [
                float(xCoord[j]) - float(xCoord[k]),
                float(yCoord[j]) - float(yCoord[k]),
                float(zCoord[j]) - float(zCoord[k]),
            ]
        )
        v_kl = np.array(
            [
        ...
        cos_delta = (np.dot(np.cross(e_ij, e_jk), np.cross(e_jk, e_kl))) / (
            np.sin(theta_ijk) * np.sin(theta_jkl)
        )
