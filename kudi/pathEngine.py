...
def get_rxCoord(lines):
    rx_coord = []
    sp_orca_line = re.compile(
        r"Single point computation of scan coordinate\s+(-?\d+\.\d+)"
    )
    sp_g09_line = re.compile(
        r"^\s+Single Point computation for reaction coordinate:\s+(-?\d+\.\d+)$"
    )
...
def all_symm_orbs_energ(blocks):
    ...
    all_energ_symm = dict(list(occ_energ_symm.items()) + list(virt_energ_symm.items()))
...
def get_all_xyz(blocks, rxc, energy, option):
    ...
    for blockNum in range(0, len(blocks)):
        ...
        header_latex = (
            "\n\\begin{center}\n   XYZ coordinates for reaction coordinate:  "
            + str(rxc[0])
            + "\n\end{center}\n"
        )
        ...
        num_atoms = str(
            len(xyz[2].split("\n")) - 1
        )  # Determining how many atoms the molecule has, this is poorly done, I should really write an sp function for that
