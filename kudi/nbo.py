    ...
    for lineNum in range(0, len(filelines)):
        ...
        if "(Occupancy)   Bond orbital/ Coefficients/ Hybrids" in outline:
            for lineNum1 in range(lineNum + 1, len(filelines)):
                bond_info = re.compile(
                    "\d+\.\s+\((\d\.\d+)\)\s+(\w+)\s.*(\d+)\)\s*(\w+)\s+(\d+)-?\s?(\w*)\s*(\d*)-?\s*(\w*)\s*(\d*)"
                )
                bond_detail = re.compile(
                    "\(\s+(\d+\.\d+)\%\)\s+(\d+\.\d+)\*\s?(\w+)\s+(\d+).*s\(\s*(\d+.\d+).*p\s?(\d+\.\d+)\(\s+(\d+\.\d+)"
                )
                # lone_pair_detail = re.compile('\d+\.\s+\((\d\.\d+)\)\s+(\w+)\s.*\d+\)\s*(\w+)\s+(\d+).*p\s?(\d+\.\d+)')
                lone_pair_detail = re.compile(
                    "\d+\.\s+\((\d\.\d+)\)\s+(\w+)\s.*\d+\)\s*(\w+)\s+(\d+).*s\(\s*(\d+.\d+).*p\s?(\d+\.\d+)\(\s+(\d+\.\d+)"
                )
                ...
                if lp and match_d_lp:
                    ...
                    if type_lp in "LP":
                        nat_orbs.setdefault(atom_lp + num_lp, []).append(
                            atom_lp
                            + num_lp
                            + ": "
                            + p_perlp
